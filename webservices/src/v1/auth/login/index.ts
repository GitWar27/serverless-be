...
import { cognitoLogin } from "./methods";
import { queryOne, mysqlconnect } from "../../../utils/sql";
...
export const handler = async (event: any, _context: any) => {
  try {
    ...
    const validatorResult = await validator(loginPayload, body);
    ...
    const cognitoLoginResult = await cognitoLogin(authUserPayload);
    ...
    const response = {
      ...
      // user: result,
      session: cognitoLoginResult.data.Session,
      user_id: cognitoLoginResult.data.ChallengeParameters.USERNAME,
    };
